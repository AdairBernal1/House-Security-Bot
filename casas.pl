...
alerta_recursiva(Alerta) :-
    ...
    format(atom(Message), 'El sensor ~w se ha activado en la casa ~w en el área ~w a las ~w-~w-~w ~w:~w:~w y el nivel de alerta es ~w.', [IdSensor, Numero, Area, Year, Month, Day, Hour, Minute, Second, Alerta]),
    format(atom(Query), 'INSERT INTO alertas (mensaje, idSensor, numeroCasa, area, fecha, nivel_alerta) VALUES (\'~w\', \'~w\', \'~w\', \'~w\', \'~w-~w-~w ~w:~w:~w\', \'~w\')', [Message, IdSensor, Numero, Area, Year, Month, Day, Hour, Minute, Second, Alerta]),
    conectar_bd,
